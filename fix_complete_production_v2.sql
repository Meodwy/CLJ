DROP FUNCTION IF EXISTS complete_production(UUID);
...
BEGIN
  ...
  FOR v_lot_reservation IN
    SELECT ilr.id, ilr.inventory_lot_id, ilr.reserved_quantity, ilr.consumed_quantity, ilr.inventory_item_id, ilr.unit, ilr.order_item_id, coi.total_required_quantity, l.quantidade_disponivel, l.numero_lote
    FROM inventory_lot_reservations ilr JOIN lotes l ON l.id = ilr.inventory_lot_id JOIN compounding_order_items coi ON coi.id = ilr.order_item_id
    WHERE ilr.order_id = p_order_id AND ilr.status = 'ACTIVE' FOR UPDATE OF ilr
  LOOP
    ...
    UPDATE inventory_lot_reservations SET consumed_quantity = v_total_consumed, status = CASE WHEN v_total_consumed >= v_lot_reservation.reserved_quantity THEN 'CONSUMED'::reservation_status WHEN v_total_consumed > 0 THEN 'PARTIALLY_CONSUMED'::reservation_status ELSE 'RELEASED'::reservation_status END WHERE id = v_lot_reservation.id;
    UPDATE lotes SET quantidade_disponivel = quantidade_disponivel + v_lot_reservation.reserved_quantity, quantidade_reservada = quantidade_reservada - v_lot_reservation.reserved_quantity WHERE id = v_lot_reservation.inventory_lot_id;
  END LOOP;
  INSERT INTO compounding_status_history(clinic_id, order_id, previous_status, new_status, changed_by, reason) VALUES (v_clinic_id, p_order_id, v_current_status::TEXT, 'PRODUCTION_COMPLETED', v_user_id, 'Producao concluida');
